...
namespace ProductClient.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create(Product p)
        {
          ...
          //  HttpClient obj = new HttpClient();
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PostAsync("api/Product", content))
                {
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:5001/api/Product/" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Edit(Product p)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PutAsync("https://localhost:5001/api/Product/" + id, content1))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ActionResult> Delete(Product p)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("https://localhost:5001/api/Product/" + prid))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
